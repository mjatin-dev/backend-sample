...
export class UserController {
  ...
  @ApiCreatedResponse({
    ...
    type: SendUserInvitationRequestDto,
  })
  @Post('invite')
  async sendUserInvitation(
    @AuthedUser() authedUser: IAuthedUser,
    @Body() body: SendUserInvitationRequestDto,
  ) {
    try {
      const user = await this.userService.sendUserInvitation(body);

      return new SuccessResponseObject('Invite sent successfully!', user);
    } catch (error) {
  ...
  @ApiOperation({ summary: 'Re-Send User Invitation' })
  @ApiCreatedResponse({
    description: 'Invite sent successfully!',
    type: SendUserInvitationRequestDto,
  })
  @Get('resend-invite/:userId')
  async resendUserInvitation(
    @AuthedUser() authedUser: IAuthedUser,
    @Param('userId') userId: number,
  ) {
    try {
      const user = await this.userService.resendUserInvitation({ userId });
  ...
  @Get('cancel-invite/:userId')
  async cancelUserInvitation(
    @AuthedUser() authedUser: IAuthedUser,
    @Param('userId') userId: number,
  ) {
    try {
      const user = await this.userService.cancelUserInvitation({ userId });
