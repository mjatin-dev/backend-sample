...
import { RuleTypeEnum } from '@/data-raptor/types';
import { RuleComponentType } from '@/data-raptor/rule/dto/Enums';
import { RuleDto } from '@/data-raptor/rule/dto/rule.dto';
      ...
      const shortlistCompressedData = totalCount
          ...
          if (item.confidence_score !== 0) {
            ...
            if (
              ruleIds.some((ruleId) => {
                ...
                if (index > -1) {
                  if (rules[index].type === RuleTypeEnum.DataValidation) {
                    // we should get this data from frontend side.ex,data validation.duplication detect
                    return true;
                  }
                }
                return false;
        ...
        .map((item) => {
          ...
          ruleIds.forEach((ruleId) => {
            ...
            const columns = (rule.rule as RuleDto).where
              .filter(
                (where) => where.type === RuleComponentType.ROOT_CONDITIONAL,
              )
